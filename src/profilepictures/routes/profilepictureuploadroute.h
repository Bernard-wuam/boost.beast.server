...
#include "profilepictures/db/updateprofilepicturedb.h"
...
namespace ProfilePictureUploadRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline profilePictureUploadRoute(boost::beast::tcp_stream
                                                                 &socket_stream,
                                                             boost::beast::http::
                                                                 request_parser<
                                                                     boost::beast::
  ...
  filePath.append(profileImageDirectory).append(filename);

  std::cout << filePath.c_str() << std::endl;
  // the "http://localhost:5333/api/posts/image/"
  // should be passed to the function for a generic usage
  // but for simplicity i would leave it this way for now.

  auto image =
      std::string{"http://localhost:5333/api/users/profilepicture/"} + filename;
  ...
  auto updateProfilePictureDbAction = co_await (
      UpdateProfilePicturedb::updateProfilePicturedb(connPool, image, user));

  if (!updateProfilePictureDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        updateProfilePictureDbAction.error());

  co_return JsonResponse::sendJsonResponse(
      requestString.get().version(), requestString.get().keep_alive(),
      boost::json::object{{"result", image}});
};
} // namespace ProfilePictureUploadRoute
