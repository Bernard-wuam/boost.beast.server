...
namespace GetProfilePictureRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::file_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline getProfilePictureRoute(boost::beast::
                                                              tcp_stream &
                                                                  socket_stream,
                                                          boost::beast::http::
                                                              request_parser<
  ...
  // http://localhost:5333/api/users/profilepicture/4d6d4d67-f87b-4beb-8bf7-c598f6cbbcfc.jpeg

  std::cout << target << " : the target" << std::endl;

  auto rIndex = target.find("profilepicture");
  ...
  if (ec) {
    ...
    path.append("assets").append("profilepicture").append("no_image.jpg");
