...
#include "users/db/loginuserdb.h"
...
namespace LoginUserRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline loginUserRoute(boost::beast::tcp_stream
                                                      &socket_stream,
                                                  boost::beast::http::
                                                      request_parser<
                                                          boost::beast::http::
  ...
  if (ec) {
    std::cerr << "error from reading inside /api/auth/login" << std::endl;
  ...
  auto userExpectedObj =
      Utility::convertBodyToObject<LoginUserSchema>(requestString.get());

  if (!userExpectedObj.has_value()) {
    JsonErrorRequestObjectFormatSchema<LoginUserSchema> loginErrorFormat;
    co_return JsonResponse::sendJsonBadObjectFormatResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        loginErrorFormat);
  }

  auto loginUser = userExpectedObj.value();
  auto loginUserDbAction =
      co_await LoginUserDB::loginUserDB(connPool, loginUser);

  if (!loginUserDbAction.has_value()) {
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        loginUserDbAction.error());
  }

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           loginUserDbAction.value());
}
} // namespace LoginUserRoute