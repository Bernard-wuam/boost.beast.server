...
#include "category/db/getcategorydb.h"
...
namespace GetCategoryRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline getCategoryRoute(boost::beast::tcp_stream
                                                    &socket_stream,
                                                boost::beast::http::
                                                    request_parser<
                                                        boost::beast::http::
  ...
  auto getCategoryDbAction =
      (co_await GetCategoryDB::getCategoryDB(connPool));

  if (!getCategoryDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        getCategoryDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           getCategoryDbAction.value());
}
} // namespace GetCategoryRoute