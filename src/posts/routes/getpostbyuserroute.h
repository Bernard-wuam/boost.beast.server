...
#include "posts/db/getpostbyuserdb.h"
...
namespace GetPostByUserRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline getPostByUserRoute(boost::beast::tcp_stream
                                                          &socket_stream,
                                                      boost::beast::http::
                                                          request_parser<
                                                              boost::beast::
  ...
  std::cout << urlParams.contains("user") << " : does it contain user"
            << std::endl;
  auto val = urlParams.find("user");
  auto category = (*val).value;
  auto getPostByUserDbAction =
      (co_await GetPostByUserDB::getPostByUserDB(connPool, category));

  if (!getPostByUserDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        getPostByUserDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           getPostByUserDbAction.value());
}
} // namespace GetPostByUserRoute