...
#include "posts/db/deletepostdb.h"
...
namespace DeletePostRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline deletePostRoute(boost::beast::tcp_stream
                                                       &socket_stream,
                                                   boost::beast::http::
                                                       request_parser<
                                                           boost::beast::http::
  ...
  auto deletePostDbAction = (co_await DeletePostDB::deletePostDB(
      connPool, indexExpected.value(), user));

  if (!deletePostDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        deletePostDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           deletePostDbAction.value());
}
} // namespace DeletePostRoute