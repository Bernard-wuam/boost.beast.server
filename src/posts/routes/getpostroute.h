...
namespace GetPostRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline getPostRoute(boost::beast::tcp_stream
                                                    &socket_stream,
                                                boost::beast::http::
                                                    request_parser<
                                                        boost::beast::http::
  ...
  auto getPostDbAction =
      (co_await GetPostDB::getPostDB(connPool, indexExpected.value(), user));

  if (!getPostDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        getPostDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           getPostDbAction.value());
}
} // namespace GetPostRoute