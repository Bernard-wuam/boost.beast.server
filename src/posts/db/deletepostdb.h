...
public:
  ...
  deletePostDB(boost::mysql::connection_pool &conn, const std::string &index,
               const std::optional<JwtUserInfoSchema> &user) {
    boost::beast::error_code ec;

    if (!user.has_value()) {
      co_return std::unexpected(makeErrorCode(AppError::Unauthorize_Access));
    }
    ...
    if (user.value().public_id == std::get<0>(result.rows<1>()[0]) ||
        user.value().role == "admin") {
      boost::mysql::results result;

      co_await getCon->async_execute(
          boost::mysql::with_params("DELETE FROM posts "
      ...
      co_return boost::json::object{{"result", "deleted"}};
    }
