...
#include "comments/db/getpostcommentdb.h"
...
namespace GetPostCommentRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline getPostCommentRoute(boost::beast::tcp_stream
                                                    &socket_stream,
                                                boost::beast::http::
                                                    request_parser<
                                                        boost::beast::http::
...
std::cout << indexExpected.value() << std::endl;
  ...
  auto getCommentDbAction =
      (co_await GetPostCommentDB::getPostCommentDB(connPool, indexExpected.value(), user));

  if (!getCommentDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        getCommentDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           getCommentDbAction.value());
}
} // namespace GetPostCommentRoute