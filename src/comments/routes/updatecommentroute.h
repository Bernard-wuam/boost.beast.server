...
#include "comments/db/updatecommentdb.h"
#include "comments/schemas/updatecommentschema.h"
...
namespace UpdateCommentRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline updateCommentRoute(boost::beast::tcp_stream
                                                          &socket_stream,
                                                      boost::beast::http::
                                                          request_parser<
                                                              boost::beast::
  ...
  auto bodyVal =
      Utility::convertBodyToObject<UpdateCommentSchema>(requestString.get());

  if (!bodyVal.has_value()) {
    JsonErrorRequestObjectFormatSchema<UpdateCommentSchema> errformat;
  ...
  auto updateCommentDbAction = (co_await UpdateCommentDB::updateCommentDB(
      connPool, indexExpected.value(), user,bodyVal.value()));

  if (!updateCommentDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        updateCommentDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           updateCommentDbAction.value());
}
} // namespace UpdateCommentRoute