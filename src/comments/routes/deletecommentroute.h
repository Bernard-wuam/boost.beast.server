...
#include "comments/db/deletecommentdb.h"
...
namespace DeleteCommentRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline deleteCommentRoute(boost::beast::tcp_stream
                                                       &socket_stream,
                                                   boost::beast::http::
                                                       request_parser<
                                                           boost::beast::http::
  ...
  auto deleteCommentDbAction = (co_await DeleteCommentDB::deleteCommentDB(
      connPool, indexExpected.value(), user));

  if (!deleteCommentDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        deleteCommentDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           deleteCommentDbAction.value());
}
} // namespace DeleteCommentRoute