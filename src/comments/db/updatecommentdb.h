...
class UpdateCommentDB {
public:
  static boost::asio::awaitable<
      std::expected<boost::json::value, std::error_code>,
      boost::asio::strand<boost::asio::io_context::executor_type>>
  updateCommentDB(boost::mysql::connection_pool &conn, const std::string &index,
                  const JwtUserInfoSchema &user, const UpdateCommentSchema &updateCommentSchema) {
    ...
    boost::mysql::static_results<std::tuple<>, std::tuple<>> emptyResult;

    co_await getCon->async_execute(boost::mysql::with_params(
                                       R"(
                                                    START TRANSACTION;
                                                    SELECT users.public_id from users 
    ...
    co_await getCon->async_execute(boost::mysql::with_params(
                                       R"(
                                                  UPDATE comments SET comments.content = {} 
                                                  WHERE comments.comment_public_id = {};
                                                  COMMIT;
                                                )",
                                       updateCommentSchema.content, index),
                                   emptyResult, diag,
                                   boost::asio::redirect_error(ec));
    getCon.return_without_reset();

    boost::json::value obj(boost::json::object{{"result", "updated"}});
