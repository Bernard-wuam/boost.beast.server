...
class CreateDisLikeDB {
public:
  static boost::asio::awaitable<
      std::expected<boost::json::value, std::error_code>,
      boost::asio::strand<boost::asio::io_context::executor_type>>
  createDisLikeDB(boost::mysql::connection_pool &conn, const std::string &index,
    ...
    //     result;
    boost::mysql::static_results<std::tuple<>, std::tuple<>, std::tuple<>,
                                 std::tuple<>>
        result;
    ...
    co_await getCon->async_execute(boost::mysql::with_params(
                                       R"(
                                                ...
                                                WHERE u.public_id = {0} 
                                                AND c.comment_public_id = {1};

                                                INSERT INTO comment_affinities(comment_id,user_id,comment_affinity_type_id)
                                                SELECT c.comment_id, u.user_id, 
                                                (SELECT comment_affinity_type_id from comment_affinities_type WHERE name = {2}) 
                                                FROM comments c 
                                                INNER JOIN users u ON u.public_id = {0} 
                                                WHERE c.comment_public_id = {1};                          

                                                COMMIT;
                                              )",
                                       user.public_id, index, "dislike"),
                                   result, diag,
