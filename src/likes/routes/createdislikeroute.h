...
#include "likes/db/createdislikedb.h"
...
namespace CreateDisLikeRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline createDisLikeRoute(boost::beast::
                                                              tcp_stream &
                                                                  socket_stream,
                                                          boost::beast::http::
                                                              request_parser<
  ...
  auto createDisLikeDbAction =
      (co_await CreateDisLikeDB::createDisLikeDB(
          connPool, indexExpected.value(), user));

  if (!createDisLikeDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        createDisLikeDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           createDisLikeDbAction.value());
}
} // namespace CreateDisLikeRoute