...
#include "likes/db/deletedislikedb.h"
...
namespace DeleteDisLikeRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline deleteDisLikeRoute(boost::beast::
                                                              tcp_stream &
                                                                  socket_stream,
                                                          boost::beast::http::
                                                              request_parser<
  ...
  auto deleteDisLikeDbAction =
      (co_await DeleteDisLikeDB::deleteDisLikeDB(
          connPool, indexExpected.value(), user));

  if (!deleteDisLikeDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        deleteDisLikeDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           deleteDisLikeDbAction.value());
}
} // namespace DeleteDisLikeRoute