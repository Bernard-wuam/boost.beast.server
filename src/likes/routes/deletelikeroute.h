...
#include "likes/db/deletelikedb.h"
...
namespace DeleteLikeRoute {

boost::asio::awaitable<
    boost::beast::http::response<boost::beast::http::string_body>,
    boost::asio::strand<
        boost::asio::io_context::
            executor_type>> inline deleteLikeRoute(boost::beast::
                                                              tcp_stream &
                                                                  socket_stream,
                                                          boost::beast::http::
                                                              request_parser<
  ...
  auto deleteLikeDbAction =
      (co_await DeleteLikeDB::deleteLikeDB(
          connPool, indexExpected.value(), user));

  if (!deleteLikeDbAction.has_value())
    co_return JsonResponse::sendJsonBadResponse(
        requestString.get().version(), requestString.get().keep_alive(),
        deleteLikeDbAction.error());

  co_return JsonResponse::sendJsonResponse(requestString.get().version(),
                                           requestString.get().keep_alive(),
                                           deleteLikeDbAction.value());
}
} // namespace DeleteLikeRoute